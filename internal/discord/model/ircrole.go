...
func EveryoneRolePayload(guildID string, reactCapable bool) map[string]any {
	perms := "104324740529169" // exactly what the bridge honours + MANAGE_MESSAGES (pin button + own-message deletes); no ADD_REACTIONS
	if reactCapable {
		perms = "104324740529233" // + ADD_REACTIONS (picker UI)
	}
	return map[string]any{
		...
		"unicode_emoji": nil,
		"position":      0,
		"permissions":   perms, // CHANGE_NICKNAME un-disables the Edit Server Profile nickname field; MANAGE_MESSAGES gates the client's pin button (this build predates the PIN_MESSAGES split); never USE_EXTERNAL_EMOJIS (no custom emojis exist)
		"managed":       false,
		"mentionable":   false,
		"flags":         0,
	}
