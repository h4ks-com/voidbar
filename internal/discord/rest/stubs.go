...
func (s *Server) handleSettingsPatch(w http.ResponseWriter, r *http.Request, u *storage.User) {
	...
	// the modified fields (per docs and the client's IncomingParser).
	if s.gw != nil {
		s.gw.Dispatch(u.ID, "USER_SETTINGS_UPDATE", patch)
	}
	// The PATCH responds with the full settings object, not 204: the
	// client deserializes it as ModelUserSettings (RestAPI.smali) and a
	// null body made its deserializer throw - surfaced as an
	// "unknown network error" toast on every theme change.
	if s.net != nil {
...
func (s *Server) handleSettingsProtoPatch(w http.ResponseWriter, r *http.Request, u *storage.User) {
	...
	// No gateway dispatch: the 126.21 IncomingParser has no
	// USER_SETTINGS_PROTO_UPDATE case; the HTTP response is the ack.
	writeJSON(w, http.StatusOK, map[string]any{
